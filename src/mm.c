             ...
             int swpoff) //swap offset
{
  if (pre != 0) {
    if (swp == 0) { // Non swap ~ page online
      ...
      // } // update: clear invalid setting (frame number = 0)
...
{                                         // no guarantee all given pages are mapped
  ...
  //struct framephy_struct *fpit = malloc(sizeof(struct framephy_struct)); // update: remove this line
  //int  fpn;
  int pgit = 0;
  ...
  // fpit->fp_next = frames; // update: remove this line
  ...
  for (pgit = pgnum - 1; pgit >= 0; --pgit)
  {
    *pte = 0;
    ...
    {
      #ifdef MEM_DEBUG
      perror("FATAL ERROR: vmap_page_range - frames are not allocated enough!");
      #endif
      return -1;
    }
    
    int init_pte_ret = init_pte(pte, 1, fpit->fpn, 0, 0, 0, 0);
    if (init_pte_ret < 0)
      return -1; // init PTE ERROR
    ...
    * Enqueue new usage page */
    int enlist_fifo_ret = enlist_pgn_node(&caller->mm->fifo_pgn, pgn+pgit);
...
{
  #ifdef MEM_DEBUG
  printf("Allocating a page range...\n");
  #endif
  ...
  for(pgit = 0; pgit < req_pgnum; pgit++)
  {
    if(MEMPHY_get_freefp(caller->mram, &fpn) == 0)
    {
      ...
      // ---------------------------------------------------------------------------
      // FAILED: SWAP -> GET FREE FRAME

      #ifdef MEM_DEBUG
      printf("No free frame to ALLOC! --> SWAP detected!\n");
      #endif
      ...
      // get free frame in MEMSWAP
      while (1)
      {
        #ifdef MEM_DEBUG
        printf("Lookup in SWAP %d...\n", caller->active_mswp->memphy_id);
        #endif
      ...
      // now successful getting a free (swap) frame from MEMSWAP

      // copy page from victim frame to swap frame
      __swap_cp_page(caller->mram, vicfpn, caller->active_mswp, swpfpn);

      // set memory of victim frame to NULL
      unsigned long baseaddr = vicfpn * PAGING_PAGESZ;
      for (unsigned long pgoffset = 0; pgoffset < PAGING_PAGESZ; ++pgoffset)
        MEMPHY_write(caller->mram, baseaddr + pgoffset, MEMORY_NULL_DATA);

      // update victim page's PTE
      pte_set_swap(&vicpte, caller->active_mswp->memphy_id, swpfpn*PAGING_PAGESZ);
      caller->mm->pgd[vicpgn] = vicpte;

      #ifdef MEM_DEBUG
      printf("Got swap frame: %d\n", swpfpn);
      printf("Victim page %d is swapped out: vframe %d to sframe %d\n", vicpgn, vicfpn, swpfpn);
      #endif
      
      // update the free frame got: victim frame
      fpn = vicfpn;
      // ---------------------------------------------------------------------------
   } 

   // got a free frame: fpn
   // ------------------------------------------------------------
      // create fp node
      ...
      // add fpn to frame list
      newfp_str->fp_next = *frm_lst;
      *frm_lst = newfp_str;

      #ifdef MEM_DEBUG
      printf("Got free frame to ALLOC: %d\n", fpn);
      #endif
      // ------------------------------------------------------------
  }
...
{
  #ifdef MEM_DEBUG
  printf("Remap: Start at %d, num page is %d\n", mapstart, incpgnum);
  #endif
