...
static int num_cpus;
static int done = 0;
static pthread_mutex_t cpu_lock; // update: add mutex lock
// int running_cpus;
// int num_cpus_needed;
...
struct mmpaging_ld_args {
	...
	struct memphy_struct *mswp; // update: remove an asterisk (*)
...
static void * cpu_routine(void * args) {
	...
	while (1) {
		...
		if (proc == NULL) {
			...
			// update: or the current priority queue is out of slot
		...
		{
			// update: queue out of slot condition
			pthread_mutex_lock(&cpu_lock);
			int slot_left = *proc->numslots;
			...
			if (slot_left == 0)
			{
				...
				id, proc->pid);
				#ifdef SCHED_DEBUG
				printf("\t\tBecause I am out of slot\n");
				#endif
				put_proc(proc);
				proc = get_proc();
				time_left = 0;
		...
		time_left--;
		pthread_mutex_lock(&cpu_lock);
		(*proc->numslots)--; // update: add decrement of proc numslots
		// running_cpus = 0;
		
		#ifdef SCHED_DEBUG
		printf("\n\tCPU%d: Time slice left = %d\n", id, time_left);
		printf("\tCPU%d: Queue %d slot left = %d\n\n", id, proc->prio, *proc->numslots);
		#endif
		pthread_mutex_unlock(&cpu_lock);
		next_slot(timer_id);
	}
...
static void * ld_routine(void * args) {
	...
	struct memphy_struct* mswp = ((struct mmpaging_ld_args *)args)->mswp; // update: remove an asterisk (*)
...
int main(int argc, char * argv[]) {
	...
	pthread_mutex_init(&cpu_lock, NULL); // update: add mutex init for cpu_lock
	// running_cpus = 0;
	// num_cpus_needed = 0;
	...
	for(sit = 0; sit < PAGING_MAX_MMSWP; sit++) {
		   ...
		   mswp[sit].memphy_id = sit; // update: add ID for MEMSWAPs
	}
	...
	mm_ld_args->mswp = (struct memphy_struct *) mswp; // update: remove an asterisk (*), address-of operator (&)
