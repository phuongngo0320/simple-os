...
// -----------------------------------------------------------------------------------
// fix: PAGING_CPU_BUS_WIDTH --> (1 << PAGING_CPU_BUS_WIDTH)
...
/* PTE BIT SWAPPED */
#define PAGING_PAGE_SWAPPED(pte) (pte&PAGING_PTE_SWAPPED_MASK) // update: add swapped bit extraction
...
// -----------------------------------------------------------------------------------
// extract page number (used in pg_getpage, free_pcb_memph)
...
#define OVERLAP(x1,x2,y1,y2) (((y2-x1)*(x2-y1)>0)?1:0) // update: remove '='

// update: add macros for memory data
#define MEMORY_INIT_DATA '0'
#define MEMORY_NULL_DATA '\0'
...
int pgwrite(
		...
		uint32_t offset);
int pg_getval(struct mm_struct *mm, int addr, BYTE *data, struct pcb_t *caller); // update: add declaration
int pg_setval(struct mm_struct *mm, int addr, BYTE value, struct pcb_t *caller); // update: add declaration
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller); // update: add declaration
